...
class AdminDAO:
    ...
    def update(self, admin_id, u_name, u_lastname, u_email, u_password, u_address, u_city, u_region, u_phone, u_age):
        cursor = self.conn.cursor()
        query = "with value as (select u_id from admin where admin_id = %s) update users set u_name = %s, u_lastname = %s, u_email = %s, u_password = %s, u_address = %s, u_city = %s, u_region = %s, u_phone = %s, u_age, u_phone = %s, u_age = %s where u_id = (select u_id from value);"
